...
export const repairCommand = new SlashCommandBuilder()
  .setName("repair")
  .setDescription("Repair a bad movie, TV episode, or TV season")
  .addSubcommand((sub) =>
    sub
      .setName("movie")
  ...
  .addSubcommand((sub) =>
    sub
      .setName("season")
      .setDescription("Report and repair an entire TV season")
      .addStringOption((opt) => opt.setName("title").setDescription("Series title").setRequired(true).setAutocomplete(true))
      .addIntegerOption((opt) => opt.setName("season").setDescription("Season number (0 = Specials)").setRequired(true).setMinValue(0))
      .addStringOption((opt) => {
        opt.setName("reason").setDescription("What's wrong?").setRequired(true);
        for (const [name, value] of reasons) opt.addChoices({ name, value });
        return opt;
